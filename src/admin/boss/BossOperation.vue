...
export default {
    ...
    methods: {
        updateData() {
            const _this = this ;
            axios.get(axios.defaults.baseURL + "/controller/admin/boss/users",
                {
                    headers: {
        ...
        levelChange(user, newLevel, account) {
            ...
            if ( confirm( msg ) ){
                axios.post(axios.defaults.baseURL + "/controller/admin/boss/users/" + user.id
                    , { level : newLevel },
                    {
                        headers: {
                            token: store.state.token
        ...
        blockUser(block, id, account) {
            ...
            if ( block ) {
                ...
                if ( confirm( msg ) ){
                    axios.put(axios.defaults.baseURL + "/controller/admin/boss/users/" + id,{},
                        {
                            headers: {
                                token: store.state.token
                            }
                ...
                if ( confirm( msg ) ){
                    axios.delete(axios.defaults.baseURL + "/controller/admin/boss/users/" + id,
                        {
                            headers: {
                                token: store.state.token
                            }
